class EcfsController < ApplicationController
  ...
  def show
    ...
    @ecf.decisions.each do |decision|
      ...
      @decisions_ecfs[decision.module_code] = [decision.outcome, decision.requested_action, decision.requested_action != "DEX - Deadline Extension" || decision.requested_action != "NP  - No penalty for late submission"  ? nil : decision.extension_date]
    end
  end
